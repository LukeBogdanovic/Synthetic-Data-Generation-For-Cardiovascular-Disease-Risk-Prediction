...
import pandas as pd
import pynvml
from Final_models.Classifier.preprocessing_utils import reorder_features, evaluate_on_test


class ECGBlock(nn.Module):
    def __init__(self, in_ch, out_ch, k):
        super().__init__()
        self.conv1 = nn.Conv1d(in_ch, out_ch, kernel_size=k, padding=k//2)
        self.bn1   = nn.BatchNorm1d(out_ch)
        self.conv2 = nn.Conv1d(out_ch, out_ch, kernel_size=k, padding=k//2)
...
class GatedFusion(nn.Module):
    def __init__(self, ecg_dim, risk_dim, hidden=64):
        super().__init__()
        self.gate = nn.Sequential(
            nn.Linear(ecg_dim+risk_dim, hidden),
            nn.ReLU(),
            nn.Linear(hidden, ecg_dim),
            nn.Sigmoid()
        )
    
    def forward(self, ecg_feat, risk_feat):
        joint = torch.cat([ecg_feat, risk_feat], dim=1)
        g = self.gate(joint)
        return torch.cat([g*ecg_feat, risk_feat], dim=1)
...
class Classifier(nn.Module):
    ...
    def __init__(self, ecg_length, num_risk_factors, num_classes):
        ...
        # Data fusion trunk
        self.fusion = GatedFusion(ecg_dim=128,risk_dim=32)
    ...
    def forward(self, ecg, risk):
        ...
        fused_features = self.fusion(ecg_features, risk_features)
...
if __name__ == "__main__":
    dataset = 0.0  # Set dataset real data percentage
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    num_epochs = 10  # Number of epochs
    BATCH_SIZE = 64
    df = pd.read_csv("augmented_dataset.csv")  # Read oversampled CRFs
    synth_data = torch.load(
        f"./Final_models/Classifier/synth_datasets/{dataset}_real_synth_dataset.pth", weights_only=False)  # Load dataloaders from pytorch file
    ...
    ecg_data = np.load("real_ecg.npy", allow_pickle=True)  # Load real ECGs
    crf_data = np.load("real_crf.npy", allow_pickle=True)  # Load real CRFs
