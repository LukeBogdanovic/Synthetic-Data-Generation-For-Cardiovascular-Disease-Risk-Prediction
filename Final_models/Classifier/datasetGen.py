...
real_data_fraction = 1.0 # Amount of real data to use
...
if len(tvae_crf_samples) >= 1:  # Check if the TVAE has been used to generate samples
    generator = Generator(ecg_length=640, n_leads=3,
                          latent_dim=100)  # Instantiate generator
    CGAN_model = torch.load(
        "./Final_models/CWGAN/models/UpsampleAndCNN_CWGAN/Model_1_GP_10.0_DTW_1.0/Model.pth", map_location=device, weights_only=False)  # Using best performing model
    ...
    train_ecg = generate_ecgs_for_crf(
        synthetic_labels_tensor.unsqueeze(1), generator, 100, BATCH_SIZE, device)  # Generate ECGs for the classes
...
torch.save(
    datasets, f"./Final_models/Classifier/synth_datasets/{real_data_fraction}_real_synth_dataset.pth")  # Save the datasets to disk in Pytorch format
