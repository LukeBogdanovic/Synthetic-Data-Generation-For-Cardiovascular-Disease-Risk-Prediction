...
def setup_filter(lowcut=0.5, highcut=40, fs=128, order=3):
...
def bandpass_filter(signal, b=None, a=None):
    '''
...
def save_generated_ecg(generator, epoch, device, lead_mins, lead_maxs, latent_dim=50, save_path="images", num_classes=4):
    ...
    if gen_ecg.shape == (1, 3, 640):
        gen_ecg = gen_ecg.transpose(0, 2, 1)
    gen_ecg = per_lead_inverse_scaling(gen_ecg, lead_mins, lead_maxs)
    gen_ecg = gen_ecg.squeeze(0)
    # gen_ecg = gen_ecg.transpose(1, 0)
    # Post-process bandpass filter
    gen_ecg = bandpass_filter(gen_ecg, b, a)
...
def init_live_plot(n_leads: int = 3, figsize: tuple[int, int] = (10, 8)) -> tuple[Figure, list[Axes], list[Line2D]]:
    plt.ion()
    fig, axs = plt.subplots(n_leads, 1, figsize=figsize, sharex=True)
    if n_leads == 1:
        axs = [axs]
    lead_labels: list[str] = ['Lead III', 'Lead V3', 'Lead V5'][:n_leads]
    lines: list[Line2D] = []
...
def plot_generated_sample(generated_signal: torch.Tensor, lead_mins, lead_maxs, fig: Figure, axs: Sequence[Axes], lines: Sequence[Line2D], amount_signal: int = 320) -> None:
    '''
    ...
    '''
    if amount_signal > 640:
        print(
            f"Amount signal: {amount_signal} greater than length of generated signal! Decreasing to valid length.")
        amount_signal = 640
    elif amount_signal < 128:
        print(
            f"Amount signal: {amount_signal} lower than 1s of signal time! Increasing to 1s.")
        amount_signal = 128
    sample = generated_signal.detach(
    ).cpu().numpy()  # Convert tensor to numpy array
    sample = per_lead_inverse_scaling(
        # Reverse scaling for the leads generated
        sample, lead_mins, lead_maxs)
    sample = sample.squeeze(
        0)  # Remove batch size dimension
    # Post-process filter the signals
    sample = bandpass_filter(sample, b, a)
    # Subplot the leads
    x = np.arange(amount_signal)
    n_leads = len(lines)
    for i in range(n_leads):
        y = sample[:amount_signal, i]
