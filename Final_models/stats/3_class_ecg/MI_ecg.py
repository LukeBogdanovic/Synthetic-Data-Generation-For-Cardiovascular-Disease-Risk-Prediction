...
dataset    = TensorDataset(ecg_tensor, mi_labels)
...
print(f"\nAccuracy on PTB-XL MI samples (ECG-only): {correct}/{n_loaded} ({correct/n_loaded*100:.1f}%)")
print("\nPrediction breakdown:")
print(results['predicted_label'].value_counts().to_string())

results.to_csv("ptbxl_mi_ecg_only_results.csv", index=False)
print("\nSaved: ptbxl_mi_ecg_only_results.csv")