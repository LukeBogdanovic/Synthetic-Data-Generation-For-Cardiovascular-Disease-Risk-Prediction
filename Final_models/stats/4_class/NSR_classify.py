...
# ── Step 1: Load PTB-XL database & find NSR records ──────────────────────────
...
nsr_codes = scp_df[
    scp_df['diagnostic_class'].str.upper().str.contains('NORM', na=False) |
    scp_df.index.str.upper().str.contains('NORM', na=False)
].index.tolist()

print(f"NSR SCP codes found: {nsr_codes}")

def has_nsr(scp_str):
    try:
        codes = eval(scp_str)
        return any(c in nsr_codes for c in codes.keys())
    except Exception:
        return False

nsr_records = ptbxl_db[ptbxl_db['scp_codes'].apply(has_nsr)].reset_index(drop=True)
print(f"Total NSR records in PTB-XL: {len(nsr_records)}")

nsr_sample = nsr_records.sample(n=N_SAMPLES, random_state=42).reset_index(drop=True)
print(f"\nSampled {N_SAMPLES} NSR records:")
print(nsr_sample[['ecg_id', 'patient_id', 'scp_codes', 'age', 'sex']].to_string(index=False))
...
def load_ptbxl_ecg(row, ptb_path):
    ...
    return record.p_signal.astype(np.float32)  # (samples, 12)
...
for _, row in nsr_sample.iterrows():
    try:
...
if len(ecg_signals) == 0:
    raise RuntimeError(
        ...
        f"  Example expected path: {os.path.join(PTB_XL_PATH, nsr_sample.iloc[0]['filename_lr'])}"
    )
...
nsr_crfs = []
attempts = 0
while len(nsr_crfs) < N_SAMPLES and attempts < 50:
    y_cand, x_cand, _ = sample_crfs(tvae, n=N_SAMPLES * 10,
                                     condition_mapping=CONDITION_MAP)
    nsr_mask = (y_cand == NSR_LABEL)
    nsr_x    = x_cand[nsr_mask]
    nsr_crfs.append(nsr_x)
    attempts += 1

nsr_crf_tensor = torch.cat(nsr_crfs, dim=0)[:N_SAMPLES]
n_loaded       = ecg_tensor.shape[0]
nsr_labels     = torch.full((n_loaded,), NSR_LABEL, dtype=torch.long)

print(f"Generated CRF tensor shape: {nsr_crf_tensor.shape}")
print(f"CRF feature dim: {nsr_crf_tensor.shape[1]}")

# ── Step 4: Run through classifier ───────────────────────────────────────────

model = Classifier(num_leads=N_LEADS,
                   num_risk_factors=nsr_crf_tensor.shape[1],
                   num_classes=4).to(device)
model.load_state_dict(torch.load(CLASSIFIER_PTH, map_location=device))
model.eval()

dataset    = TensorDataset(ecg_tensor, nsr_crf_tensor, nsr_labels)
...
results['true_label']      = 'None'
results['predicted_label'] = [CLASS_NAMES[p] for p in all_preds]
results['correct']         = results['predicted_label'] == 'None'
...
print(results[['ecg_id', 'age', 'sex', 'predicted_label',
               'correct', 'prob_None', 'confidence']].to_string(index=False))

correct = results['correct'].sum()
print(f"\nAccuracy on PTB-XL NSR samples: {correct}/{n_loaded} ({correct/n_loaded*100:.1f}%)")
print("\nPrediction breakdown:")
print(results['predicted_label'].value_counts().to_string())

results.to_csv("ptbxl_nsr_classifier_results.csv", index=False)
print("\nSaved: ptbxl_nsr_classifier_results.csv")