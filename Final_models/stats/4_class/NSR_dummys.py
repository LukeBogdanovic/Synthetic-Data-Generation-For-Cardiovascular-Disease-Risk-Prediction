...
NSR_LABEL      = 0   # maps to 'None' in CLASS_NAMES
...
# ── Step 1: Load PTB-XL & find NSR records ───────────────────────────────────
...
# NSR in PTB-XL is coded as 'NORM' in scp_codes
nsr_codes = scp_df[
...
dummy_crfs = np.stack([
    build_dummy_crf(row, NSR_GROUP_MEANS, CRF_FEATURE_DIM)
...
crf_cols = list(NSR_GROUP_MEANS.keys())
...
dataset    = TensorDataset(ecg_tensor, crf_tensor, nsr_labels)
...
results.to_csv("ptbxl_nsr_dummy_crf_results.csv", index=False)
print("\nSaved: ptbxl_nsr_dummy_crf_results.csv")