...
# ── Step 1: Load PTB-XL database & find AFib records (proxy for Stroke) ──────
...
stroke_crfs = []
attempts    = 0
while len(stroke_crfs) < N_SAMPLES and attempts < 50:
    y_cand, x_cand, _ = sample_crfs(tvae, n=N_SAMPLES * 10,
                                     condition_mapping=CONDITION_MAP)
    stroke_mask = (y_cand == STROKE_LABEL)
    stroke_x    = x_cand[stroke_mask]
    stroke_crfs.append(stroke_x)
    attempts += 1

stroke_crf_tensor = torch.cat(stroke_crfs, dim=0)[:N_SAMPLES]
n_loaded          = ecg_tensor.shape[0]
stroke_labels     = torch.full((n_loaded,), STROKE_LABEL, dtype=torch.long)

print(f"Generated CRF tensor shape: {stroke_crf_tensor.shape}")
print(f"CRF feature dim: {stroke_crf_tensor.shape[1]}")

# ── Step 4: Run through classifier ───────────────────────────────────────────

model = Classifier(num_leads=N_LEADS,
                   num_risk_factors=stroke_crf_tensor.shape[1],
                   num_classes=4).to(device)
model.load_state_dict(torch.load(CLASSIFIER_PTH, map_location=device))
model.eval()

dataset    = TensorDataset(ecg_tensor, stroke_crf_tensor, stroke_labels)
...
results['true_label']      = 'Stroke'   # AFib proxy
...
print(f"\nAccuracy on PTB-XL AFib samples (stroke proxy): {correct}/{n_loaded} ({correct/n_loaded*100:.1f}%)")
print("\nPrediction breakdown:")
print(results['predicted_label'].value_counts().to_string())

results.to_csv("ptbxl_afib_stroke_proxy_tvae_results.csv", index=False)
print("\nSaved: ptbxl_afib_stroke_proxy_tvae_results.csv")