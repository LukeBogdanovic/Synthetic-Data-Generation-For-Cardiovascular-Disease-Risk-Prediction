...
# Stroke group means from SHAREEDB analysis (placeholder — replace with real values
# from shareedb_vectors_7feat.csv if available)
STROKE_GROUP_MEANS = {
    'gender': 0.67,
    'age':    0.46,
    'weight': 0.39,
...
dummy_crfs = np.stack([
    build_dummy_crf(row, STROKE_GROUP_MEANS, CRF_FEATURE_DIM)
...
crf_cols = list(STROKE_GROUP_MEANS.keys())
...
dataset    = TensorDataset(ecg_tensor, crf_tensor, stroke_labels)
...
results.to_csv("ptbxl_afib_stroke_proxy_results.csv", index=False)
print("\nSaved: ptbxl_afib_stroke_proxy_results.csv")