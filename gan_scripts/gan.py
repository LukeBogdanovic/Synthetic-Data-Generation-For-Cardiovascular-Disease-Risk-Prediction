...
def build_generator(latent_dim=100, ecg_shape=(3, 1536), crf_dim=14):
    ...
    x_noise = Conv1D(128, kernel_size=15, activation='relu',
                     padding='same')(x_noise)
    x_noise = Conv1D(64, kernel_size=15, activation='relu',
                     padding='same')(x_noise)
    gen_ecg = Conv1D(1536, kernel_size=15, activation='tanh',
                     padding='same')(x_noise)
...
def build_discriminator(ecg_shape=(3, 1536), crf_dim=14):
    ...
    x_ecg = Conv1D(64, kernel_size=15, activation=LeakyReLU(
        alpha=0.2), strides=2, padding='same')(ecg_input)
    x_ecg = Conv1D(128, kernel_size=15, activation=LeakyReLU(
        alpha=0.2), strides=2, padding='same')(x_ecg)
    x_ecg = Flatten()(x_ecg)
...
def train_gan(dataset, epochs=1000, batch_size=16, latent_dim=100):
    ...
    combined.compile(optimizer=Adam(1e-4, beta_1=0.5),
                     loss='binary_crossentropy', metrics=[Accuracy(), Precision(), Recall()])
    combined.summary()
    discriminator.compile(optimizer=Adam(
        1e-5, beta_1=0.5), loss='binary_crossentropy', metrics=[Accuracy(), Precision(), Recall()])
    ...
    for epoch in range(epochs):
        ...
        gen_ecgs, gen_crfs = generator.predict([noise, real_crfs])
        d_loss_real = discriminator.train_on_batch(
            [real_ecgs, real_crfs, real_crfs], np.ones((batch_size, 1)))
        d_loss_fake = discriminator.train_on_batch(
            [gen_ecgs, gen_crfs, real_crfs], np.zeros((batch_size, 1)))
        noise = np.random.normal(0, 1, (batch_size, latent_dim))
        g_loss = combined.train_on_batch(
            [noise, real_crfs], np.ones((batch_size, 1)))

        if epoch % 10 == 0:
            print(f"Epoch {epoch}:")
            print(
                f"     D Loss Real: {d_loss_real[0] :.4f} | D Loss Fake: {d_loss_fake[0] :.4f} | G Loss: {g_loss[0] :.4f}")
            print(
                f"     D Acc Real: {d_loss_real[1] :.4f} | D Acc Fake: {d_loss_fake[1] :.4f} | G Acc: {g_loss[1] :.4f}")
            print(
                f"     D Precision Real: {d_loss_real[2] :.4f} | D Precision Fake: {d_loss_fake[2] :.4f} | G Precision: {g_loss[2] :.4f}")
            print(
                f"     D Recall Real: {d_loss_real[3] :.4f} | D Recall Fake: {d_loss_fake[3] :.4f} | G Recall: {g_loss[3] :.4f}")
